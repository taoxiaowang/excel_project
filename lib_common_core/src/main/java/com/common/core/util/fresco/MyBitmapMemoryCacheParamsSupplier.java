package com.common.core.util.fresco;
...
/**
 *  author: wangtao at date :2020/10/11
 *  tag: class//fresco内存限制
 *  description:
 */
    ...
    public MemoryCacheParams get() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            return new MemoryCacheParams(
                    getMaxCacheSize(), MAX_CACHE_ENTRIES, MAX_CACHE_EVICTION_SIZE,
                    MAX_CACHE_EVICTION_ENTRIES, getMaxCacheSize() / 10);
        } else {
            return new MemoryCacheParams(
                    getMaxCacheSize(),
